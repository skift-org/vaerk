...
export enum struct ProgramType : u32 {
    NONE = 0,
    LOAD = 1,
    DYNAMIC = 2,
    INTERP = 3,
...
export enum struct ProgramFlags : u32 {
    READ = 1,
    WRITE = 2,
    EXEC = 4,
    SHARED = 32,
...
export struct [[gnu::packed]] ProgramHeader {
    ProgramType type;
    u32 flags;
    u64 offset;
    u64 vaddr;
...
export struct [[gnu::packed]] Symbol {
    u32 name;
    u8 info;
    u8 other;
    u16 shndx;
...
export struct [[gnu::packed]] Ident {
    u8 magic[4];
    u8 klass;
    u8 data;
    u8 version;
...
export struct [[gnu::packed]] ImageHeader {
    Ident ident;
